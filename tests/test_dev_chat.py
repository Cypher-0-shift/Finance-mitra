...
def test_dev_chat_endpoint_404_in_production(monkeypatch):
    """Hard security gate test: confirm /dev/chat returns 404 when ENVIRONMENT=production."""
    ...
    client = TestClient(app.main.app)
    payload = {
        "session_id": "test_prod_attempt",
        "message": "Can I test without WhatsApp signatures?",
        "input_type": "text",
        "language": "en",
    }
    response = client.post("/dev/chat", json=payload)

    # Must return 404 Not Found because router is completely omitted from app route table in production
    assert response.status_code == 404, f"Expected 404 Not Found in production, got {response.status_code}"
